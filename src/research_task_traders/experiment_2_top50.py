...
OUTPUT_FILE = RESULT_DIR / "experiment_2_top50_benchmark.csv"
...
def main():
    ...
    # -----------------------------------------------------------------------
    # Top 50 only: rank 1-50 from the original Top-100 formation table
    # -----------------------------------------------------------------------
    ...
    for row in top100_rows:
        ...
        # Experiment 2 rule:
        # keep only original market-cap ranks 1 through 50.
        if rank > TOP_N:
            ignored_bottom50 += 1
            continue
        ...
        top50_by_year.setdefault(
            year,
            [],
        ).append(total_return)

    if not top50_by_year:
        raise RuntimeError(
            "No usable Top-50 returns found."
        )
    ...
    if not years:
        raise RuntimeError(
            "Top-50 and SPY data have no matching years."
        )

    for year in years:
        stock_returns = top50_by_year[year]
        spy_return = spy_by_year[year]

        top50_mean = statistics.fmean(
            stock_returns
        )

        top50_median = statistics.median(
            stock_returns
        )
        ...
        output_rows.append(
            {
                "period": str(year),
                "n": len(stock_returns),
                "top50_mean_pct": round2(
                    top50_mean
                ),
                "top50_median_pct": round2(
                    top50_median
                ),
    ...
    overall_top50_mean = statistics.fmean(
        all_top50_returns
    )

    overall_top50_median = statistics.median(
        all_top50_returns
    )
    ...
    output_rows.append(
        {
            "period": "Overall",
            "n": len(all_top50_returns),
            "top50_mean_pct": round2(
                overall_top50_mean
            ),
            "top50_median_pct": round2(
                overall_top50_median
            ),
            "spy_return_pct": round2(
                overall_spy_mean
            ),
    ...
    print()
    print("Experiment 2 - Top 50 vs SPY")
    print("-" * 90)
    ...
    for row in output_rows[:-1]:
        print(
            f"{row['period']}: "
            f"N={row['n']:2d} | "
            f"Top50 mean={row['top50_mean_pct']:+7.2f}% | "
    ...
    print(
        f"N={overall['n']} | "
        f"Top50 mean={overall['top50_mean_pct']:+.2f}% | "
        f"Top50 median={overall['top50_median_pct']:+.2f}% | "
    ...
    print()
    print(
        f"Ignored rank 51-100 rows: "
        f"{ignored_bottom50:,}"
    )

    if error_count:
        print(
            f"Warning: {error_count} Top-50 error rows "
            "were excluded."
        )
    ...
    print()
    print(
        f"Saved Experiment 2 to: {OUTPUT_FILE}"
    )
